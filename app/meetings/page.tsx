...
function MeetingCard({ meeting, isOpen, onToggle, onDeleteRequest }: { meeting: Meeting; isOpen: boolean; onToggle: (open: boolean) => void; onDeleteRequest: (meeting: Meeting) => void }) {
  ...
  return (
    <article className={`overflow-hidden rounded-2xl border bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] transition hover:border-[#c9c8c5] ${isSampleMeeting ? "sample-meeting-card" : "border-[#deddda]"}`}>
      <button type="button" aria-expanded={isOpen} aria-controls={`meeting-panel-${meeting.id}`} onClick={() => onToggle(!isOpen)} className="grid w-full cursor-pointer gap-5 px-5 py-5 text-left focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-inset focus-visible:ring-[#e9e9e7] sm:grid-cols-[minmax(0,1fr)_auto] sm:px-6">
...
function MeetingsContent() {
  ...
  const hasActiveMeetingFilters = filter !== "all" || Boolean(searchQuery.trim());
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f5] text-[#191919]">
      ...
      {meetingPendingDeletion && <div className={`fixed inset-0 z-40 grid place-items-center bg-[#191919]/20 px-5 py-8 backdrop-blur-[2px] ${isDeleteModalClosing ? "modal-backdrop-exit pointer-events-none" : "modal-backdrop-enter"}`} role="presentation" onMouseDown={closeDeleteModal}>
        <section role="dialog" aria-modal="true" aria-labelledby="delete-meeting-title" className={`w-full max-w-md rounded-xl border border-[#deddda] bg-white p-5 shadow-[0_1px_2px_rgba(0,0,0,0.03)] sm:p-6 ${isDeleteModalClosing ? "modal-panel-exit" : "modal-panel-enter"}`} onMouseDown={(event) => event.stopPropagation()}>
          ...
          <div className="mt-6 flex justify-end gap-2">
            <button type="button" onClick={closeDeleteModal} className="cursor-pointer rounded-md px-3 py-2 text-[13px] font-medium text-[#625f5c] transition hover:bg-[#ececea] focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#e9e9e7]">Cancel</button>
            <button type="button" onClick={confirmDelete} className="cursor-pointer rounded-md bg-[#a8342a] px-3.5 py-2 text-[13px] font-medium text-white transition hover:bg-[#8f2c23] focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[#f7dfdc]">Delete meeting</button>
          </div>
        </section>
      </div>}
    </div>
